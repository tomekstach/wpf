...
// AstoSoft - start - change the template
$template_version = (int) $request->variable('style', 0);
if ($user->data['user_id'] !== ANONYMOUS && ($template_version === 2 or $template_version === 3)) {
  $user_id = (int) $user->data['user_id'];
  $sql = 'UPDATE ' . USERS_TABLE . ' SET user_style = ' . $template_version . ' WHERE user_id = ' . $user_id;
  $result = $db->sql_query($sql);
  $user->data['user_style'] = $template_version;
}
// AstoSoft - end - change the template
